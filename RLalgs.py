...
tweets.regular_node_set_unique = tweets.regular_node_set_unique.apply(lambda txt: set([int(i) for i in txt.strip("'[").strip("]'").split() if i.isdigit()]))

# all the contexts
twitter_contexts = tweets.context
# needed as vector for the regressions
twitter_contexts = list(map(lambda context: np.array(context.split(), dtype=float), twitter_contexts))
...
for seed in seeds.keys():
    ...
    seeds[seed] = [twitter_contexts[idx] for idx in context_idx]


def get_reward(context, influencer_idx):
    influencer = INFLUENCERS[influencer_idx]
    rewards = tweets.loc[tweets.context == ' '.join(context[:DCONTEXT+1].astype(str)),:].loc[tweets.influencer == influencer]
...
def get_tweet(context, influencer_idx):
    influencer = INFLUENCERS[influencer_idx]
    tweet = tweets.loc[tweets.context == ' '.join(context[:DCONTEXT+1].astype(str)),:].loc[tweets.influencer == influencer]
    if tweet.size > 0:
        tweet = tweet.sample()
    return tweet
...
def linucb_reward_and_selections(seed, campaign, contexts, lognorm=False):
    ...
    exploration_factor = np.sqrt(np.log(2*BUDGET*K/delta) / 2)
    ...
    for tt in range(T):
        ...
        V = [np.diag(np.ones(DCONTEXT)) for _ in np.arange(K)]
        observed_reward = [np.zeros(DCONTEXT) for _ in np.arange(K)]

        prev_activated = set()

        # initialization phase (with separate contexts)
        for k in np.arange(K):
            context = contexts[tt * H + k]
        ...
        for t in np.arange(K, BUDGET):
            ...
            for k in np.arange(K):
                ...
                theta_estimators[t,k] = preprocessing.normalize(theta_estimators[t,k][:,np.newaxis], axis=0, norm='l2').ravel()
                reward_estimators[t,k] = theta_estimators[t,k].dot(context) + exploration_factor * np.sqrt(context.dot(inv_V).dot(context))
        ...
        for t in np.arange(K, H):
            ...
            for k in np.arange(K):
                ...
                if selections_hist[k] == 0:
                    ...
                    gt = 1 / float(selections_hist[k]**2) * np.sum([ activations_hist_fatgt[k][s] * s for s in np.arange(selections_hist[k]) ])
                    beta = (1 + np.sqrt(2)) * np.sqrt(np.sum([ activations_hist_fatgt[k][s] * s for s in np.arange(selections_hist[k]) ]) * np.log(4 * t) / float(selections_hist[k]**2 * (selections_hist[k] + 1))) + np.log(4 * t) / float(selections_hist[k]) / 3
                b_k = gt + beta
...
def lsvi_reward_and_selections(seed, campaign, contexts):
    ...
    for t in np.arange(0, T):
        ...
        for h in np.arange(H - 1, -1, -1):
            ...
            theta_estimators[t][h] = preprocessing.normalize(theta_estimators[t][h][:,np.newaxis], axis=0, norm='l2').ravel()
    ...
    for t in np.arange(0, T):
        ...
        for h in np.arange(H - 1, -1, -1):
            for k in np.arange(K):
                ...
                inv_Sigma = np.linalg.inv(design_matrix[h][k])
                x = list([phi(tau, h, k) * training_rewards[tau] if actions_hist[tau][h][k] == 1 else np.zeros(d) for tau in np.arange(t)])
                x.append(np.zeros(d))
                theta_estimators[t][h][k] = inv_Sigma.dot(sum(x))
                theta_estimators[t][h][k] = np.nan_to_num(theta_estimators[t][h][k])
                theta_estimators[t][h][k] = preprocessing.normalize(theta_estimators[t][h][k][:,np.newaxis], axis=0, norm='l2').ravel()
    ...
    for tt in np.arange(T):
        ...
        for h in np.arange(H - 1, -1, -1):
            for k in np.arange(K):
                ...
                x = list([phi(tau, h, k) * training_rewards[tau] if actions_hist[tau][h][k] == 1 else np.zeros(d) for tau in np.arange(tt)])
                x.append(np.zeros(d))
                theta_estimators[tt][h][k] = inv_Sigma.dot(sum(x))
                theta_estimators[tt][h][k] = np.nan_to_num(theta_estimators[tt][h][k])
                theta_estimators[tt][h][k] = preprocessing.normalize(theta_estimators[tt][h][k][:,np.newaxis], axis=0, norm='l2').ravel()
        ...
        for t in np.arange(K if tt == 0 else 0, H):
            ...
            for k in np.arange(K):
                ...
                if selections_hist[k] == 0:
                    ...
                    gt = 1 / float(selections_hist[k]**2) * np.sum([ activations_hist_glm[k][s] * s for s in np.arange(selections_hist[k]) ])
                    beta = (1 + np.sqrt(2)) * np.sqrt(np.sum([ activations_hist_glm[k][s] * s for s in np.arange(selections_hist[k]) ]) * np.log(4 * t) / float(selections_hist[k]**2 * (selections_hist[k] + 1))) + np.log(4 * t) / float(selections_hist[k]) / 3
                # gts and betas
                gts[t][k].append(gt)
                betas[t][k].append(beta)
                # average gts and betas
...
def execute_algs(seed):
	...
	activations_hist_lsvigt_all, influencer_hist_lsvigt, elapsed_time = rl_lsvigt_reward_and_selections(seed, tweets, contexts)
	...
	activations_hist_lsvi_all, influencer_hist_lsvi, elapsed_time = lsvi_reward_and_selections_septhetas(seed, tweets, contexts)
	to_add = pd.DataFrame({'reward': np.cumsum(activations_hist_lsvi_all)})
	to_add['seed'] = seed
	...
	activations_hist_fatgt_all, influencer_hist_fatgt, elapsed_time = fatgt_reward_and_selections(seed, tweets, contexts)
	...
	activations_hist_rlfatgt_all, influencer_hist_rlfatgt, elapsed_time = rlfatgt_reward_and_selections(seed, tweets, contexts)
